...
export class GetAllOrderAfterDateService {
  constructor(private readonly orderRepository: OrderRepositoryInterface) {}

  async findAllOrdersAfterDate(date: string): Promise<Order[]> {
    const orders = await this.orderRepository.findAllOrdersAfterDate(date);
    if (orders.length === 0) {
      throw new Exception(ExceptionTypeEnum.NotFound, `Orders after ${date} not found`);
    }
    return orders;
  }
}
